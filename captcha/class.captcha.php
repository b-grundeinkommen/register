<?php
	...
	*/
	...
	class captcha
	{
		 ...
		 * @return captcha
		 */
		function captcha($session_key, $temp_dir, $target)
		{
			$this->session_key = $session_key;
			$this->temp_dir    = $temp_dir;
			$this->captchaDir = $target;
		}
		...
		function _generate_image( $location, $char_seq )
		{
			...
			for ($i=0;$i<=$num_chars;$i++)
			{
				...
				ImageTTFText( $img, $fontsize, $angle, $start_x+$i*$max_x_ofs, $y_pos, $color, $this->captchaDir . '/default.ttf', substr($char_seq,$i,1) );
			}
		...
		/**
		 * Returns name of the newly generated or existing captcha image file
		 *
      ...
      // Check if there is already a captcha image for this session.
      $fh = @fopen($this->temp_dir . '/cap_' . $this->session_key . '.txt', "r" );
      if ($fh)
      {
      	// Session hash file found. Read the hash and see if there is an image.
      	$hash = fgets($fh);
      	if (is_readable($this->temp_dir.'/'.'cap_' . $hash . '.jpg'))
      		return $hash;
      };
    ...
    /**
     * Removes the image created by this class to avoid resuage.
     */
		function removeImage()
		{
			$hashFile = $this->temp_dir.'/' . 'cap_'.$this->session_key . '.txt';
      $fh = @fopen($hashFile, "r");
      if ($fh)
      {
      	// Session hash file found. Read the hash and see if there is an image.
      	$hash = fgets($fh);
      	$imageFile = $this->temp_dir.'/'.'cap_' . $hash . '.jpg';
