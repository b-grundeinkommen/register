...
function mk_ldap_account($name, $email)
{
	...
	$sn = str_replace(" ","", $name);
	$uid2 = str_replace("@","%",$mail);
	$userprincipalname = str_replace(" ",".",$name)."@".DOMAIN_CURRENT_SITE;
	$uid1 = str_replace(" ",".",$name)."@".DOMAIN_CURRENT_SITE;
        $uid3 = str_replace(" ","_",$name);
	$mailsn = "SN: ".$sn;
	$mailname = "\nNAME: " . $name;
	...
	$info["description"] = $description;
      ...
      // Send a confirmation mail if the user could be added to the LDAP database.
			if ($r === TRUE)
			{
				...
				$message .= "Sie sind mit Ihrem Namen (" . utf8_decode($name) . ") und folgenden Benutzerdaten freigeschaltet worden: \n";
				$message .= "Die Benutzeridentifikation (der Anmeldename) lautet je nach Anwendung: " . $mail . " oder ".$uid3."\n";
				$message .= "Das Passwort ist: " . $sendmailpasswort . "\n\n";
				$message .= "Wenn Sie Fragen haben, dann schauen Sie in der Gruppe bgeserver.admin oder auf https://".DOMAIN_CURRENT_SITE."/ vorbei.\n";
